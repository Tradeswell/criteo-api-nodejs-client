...
class Criteo_API_Client extends API_Client {
  constructor(id, secret, host = 'api.criteo.com', endpoint = '', version = '2023-04') {
    super(host);
   ...
   * @param {(number|string)} [advertiser] - Criteo advertiser ID
   * @param {function} [callback] - Optional callback
   */
   ...
   * @param {(number|string)} audience - Audience ID
   * @param {function} [callback] - Optional callback
   */
   ...
   * @param {(number|string)} [options.gumCallerId] - Required when adding audience via gum IDs.
   * @param {function} [callback] - Optional callback
   * @param {boolean} internalIdentifiers
   */
   ...
   * @param {(number|string)} adset - An ad set ID
   * @param {function} [callback] - Optional callback
   */
   ...
   * @param {(number|string)} updates[].id - Ad set ID. Auto-converted to string for convenience
   ...
   * @param {number} [updates[].attributes.targeting.frequencyCapping.maximumImpressions] - Max impressions per frequency
  ...
  updateAdSets(updates = [], callback) {
    const payload = {
      data: updates.map(update => {
        ...
        if (update.attributes.schedule && update.attributes.schedule.startDate) {
          update.attributes.schedule.startDate = new Date(
            update.attributes.schedule.startDate,
          ).toISOString();
        }
        if (update.attributes.schedule && update.attributes.schedule.endDate) {
          update.attributes.schedule.endDate = new Date(
            update.attributes.schedule.endDate,
          ).toISOString();
        }
  ...
  generateConsentURL(publicSigningKey, signingSecret, state, redirect) {
    ...
    return `https://consent.criteo.com/request${query}&signature=${sha512.hmac(
      signingSecret,
      query,
    )}`;
  }
   ...
   * @deprecated
   * @param {object} options
   * @param {(number|string)} [options.advertiserIds]
   * @param {(number|string)} [options.campaignIds]
   ...
   * @param {object} options
   * @param {(number|string)} [options.advertiserIds]
   * @param {(number|string)} [options.budgetIds]
   * @param {(number|string)} [options.categoryHashCodes]
   * @param {string} [options.bidType] - Unknown, CPC, COS, or CPO
   * @param {string} [options.campaignStatus] - Running, Archived or NotRunning
   ...
   * @param {(number|string)} campaigns[].campaignId
   ...
   * @param {(number|string)} [campaigns[].categories[].categoryHashCode]
   ...
   * @deprecated
   * @param {(number|string)} campaign - Campaign ID
   ...
   * @param {(number|string)} catalogs[].catalogId
   * @param {object[]} catalogs[].categories - An array of category objects, specifying enabled or disabled.
   * @param {(number|string)} catalogs[].categories[].categoryHashCode
  ...
  getAccounts(callback) {
    return this.criteoApiRequest({
      method: 'get',
      path: `/${this.version}/retail-media/accounts`,
      handler: this.processJSON.bind(this),
      callback: callback,
    });
  }
  ...
  getCatalogsByAccountId(accountId, callback) {
    return this.criteoApiRequest({
      method: 'post',
      path: `/${this.version}/retail-media/accounts/${accountId}/catalogs`,
      body: JSON.stringify({}),
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  getCatalogOutput(catalogId, callback) {
    return this.criteoApiRequest({
      method: 'get',
      path: `/${this.version}/retail-media/catalogs/${catalogId}/output`,
      handler: this.processResponse.bind(this),
      callback: callback,
    });
  }
  ...
  getCampaignsByAccountId(accountId, pageInfo, callback) {
    const pageQueryString = pageInfo
      ? `?pageIndex=${pageInfo.pageIndex}&pageSize=${pageInfo.pageSize}`
      : '';

    return this.criteoApiRequest({
      method: 'get',
      path: `/${this.version}/retail-media/accounts/${accountId}/campaigns${pageQueryString}`,
      handler: this.processJSON.bind(this),
      callback: callback,
    });
  }
  ...
  getCampaignById(campaignId, callback) {
    return this.criteoApiRequest({
      method: 'get',
      path: `/${this.version}/retail-media/campaigns/${campaignId}`,
      handler: this.processJSON.bind(this),
      callback: callback,
    });
  }
  ...
  createCampaignByAccountId(accountId, campaignData, callback) {
    const payload = {
      data: { type: 'RetailMediaCampaign', attributes: campaignData },
    };

    return this.criteoApiRequest({
      method: 'post',
      path: `/${this.version}/retail-media/accounts/${accountId}/campaigns`,
      body: JSON.stringify(payload),
      handler: this.processJSON.bind(this),
      callback: callback,
    });
   ...
   * @param {function} [callback] - Optional callback
   */
  getLineItemsByCampaignId(accountId, campaignId, pageInfo, callback) {
    const pageQueryString = pageInfo
      ? `&pageIndex=${pageInfo.pageIndex}&pageSize=${pageInfo.pageSize}`
      : '';

    const queryString = `?limitToCampaignId=${campaignId}${pageQueryString}`

    return this.criteoApiRequest({
      method: 'get',
      path: `/${this.version}/retail-media/accounts/${accountId}/line-items${queryString}`,
      handler: this.processJSON.bind(this),
      callback: callback,
    });
  }
  ...
  getLineItemById(lineItemId, callback) {
    return this.criteoApiRequest({
      method: 'get',
      path: `/${this.version}/retail-media/line-items/${lineItemId}`,
      handler: this.processJSON.bind(this),
      callback: callback,
    });
  }
  ...
  createLineItemByCampaignId(campaignId, lineItemData, callback) {
    ...
    return this.criteoApiRequest({
      method: 'post',
      path: `/${this.version}/retail-media/campaigns/${campaignId}/line-items`,
      body: JSON.stringify(payload),
      handler: this.processJSON.bind(this),
      callback: callback,
    });
  ...
  getReport(reportType, query, callback) {
    const payload = {
      data: { type: 'RetailMediaReportRequest', attributes: query },
    };

    return this.criteoApiRequest({
      method: 'post',
      path: `/${this.version}/retail-media/reports/${reportType}`,
      body: JSON.stringify(payload),
      handler: this.processJSON.bind(this),
      callback: callback,
    });
  ...
  getReportStatus(reportId, callback) {
    return this.criteoApiRequest({
      method: 'get',
      path: `/${this.version}/retail-media/reports/${reportId}/status`,
      handler: this.processJSON.bind(this),
      callback: callback,
    });
  }
  ...
  getReportOutput(reportId, filepath, callback) {
    ...
    return this.criteoApiRequest({
      method: 'get',
      path: `/${this.version}/retail-media/reports/${reportId}/output`,
      handler: handler,
      callback: callback,
    });
  }
